...
If a relative path, it's relative to the current working directory (cwd). For example, `{path: 'unicorn'}` would result in a storage file in `~/Library/Application Support/App Name/unicorn`.
